 ...
 * Hem server.js (Railway: /api/webhook ve /api/webhook/lemonsqueezy) hem Vercel api/webhook/* bu modülü kullanır.
 * İmza: X-Signature + LEMON_SQUEEZY_WEBHOOK_SECRET (server.js / api/webhook/*).
 * Öne çıkan olaylar: order_created, subscription_created, subscription_updated (+ ödeme, iptal vb.).
 */
...
/** Hoş geldin mailinde görünen plan adı (ör. monthly_plan_pro → Pro Plan) */
function lemonPlanDisplayName(planSnap) {
  const p = String(planSnap || "").trim().toLowerCase();
  if (p === "pro" || p === "monthly_plan_pro") return "Pro Plan";
  if (p === "monthly_plan") return "Aylık Plan";
  if (p === "yearly_plan") return "Yıllık Plan";
  if (p === "enterprise" || p.startsWith("enterprise_")) return "Kurumsal";
  if (p === "addon") return "Ek Paket";
  return planSnap ? String(planSnap) : "Plan";
}
 ...
 * @param {function} [deps.ensureUserRow] (authUserId, email) => Promise<void> — public.users satırı yoksa oluştur
 */
async function runLemonWebhook(body, deps) {
  ...
  async function resolveLemonWebhookUserId(b) {
    ...
    const emailCandidates =
      Array.isArray(sum.userEmails) && sum.userEmails.length > 0
        ? sum.userEmails
        : sum.userEmail
          ? [sum.userEmail]
          : [];

    const primaryEmail = (emailCandidates[0] || sum.userEmail || "").trim();
    if (sum.customUserId && primaryEmail && typeof deps.ensureUserRow === "function") {
      await deps.ensureUserRow(sum.customUserId, primaryEmail);
    }
    ...
    if (sum.customUserId) {
      ...
      console.warn(
        "[Lemon] custom user_id veritabanında yok:",
        sum.customUserId,
        "→ e-posta adaylarıyla eşleştiriliyor:",
        emailCandidates.length ? emailCandidates.join(", ") : "(yok)"
      );
    }

    for (let i = 0; i < emailCandidates.length; i++) {
      const em = emailCandidates[i];
      const u = await getUserByEmail(em);
      if (u) {
        console.log("[Lemon] Kullanıcı eşleşti: e-posta →", u.id, em);
        return u.id;
      }
    }

    if (emailCandidates.length) {
      console.warn("[Lemon] Hiçbir e-posta adayı DB ile eşleşmedi:", emailCandidates.join(", "));
    }
    return null;
  }
  ...
  async function applyLemonPaidPlanToUser(uid, planSnap, opts) {
    ...
    if (opts.sendWelcomeEmail && !opts.renewal && planSnap !== "addon" && after && after.email) {
      try {
        const { sendEmail } = require("./sendWelcomeEmail");
        await sendEmail(after.email, lemonPlanDisplayName(after.plan || planSnap), {
          planCredits: grant,
          balanceCredits: after.credits ?? FREE_CREDITS
        });
      } catch (e) {
        console.warn("[Lemon] Hoş geldin e-postası:", e && e.message);
      }
    }
  }
  ...
  try {
    switch (sum.eventName) {
      case "subscription_created":
        if (uid) {
          await applyLemonPaidPlanToUser(uid, planSnap, {
            subscriptionId: sum.resourceId,
            renewal: false,
      ...
      case "order_created":
            ...
            const emails =
              Array.isArray(sum.userEmails) && sum.userEmails.length > 0
                ...
                : sum.userEmail
                  ? [sum.userEmail]
                  : [];
            for (let ei = 0; ei < emails.length && !targetId; ei++) {
              const uEm = await getUserByEmail(emails[ei]);
              if (uEm) targetId = uEm.id;
            }
          }
          if (!targetId) {
            console.warn("[Lemon] order_created addon: kullanıcı yok (id / custom user_id / e-posta)");
            break;
          }
        ...
        if (uid) {
          await applyLemonPaidPlanToUser(uid, planSnap, { renewal: false, sendWelcomeEmail: true });
