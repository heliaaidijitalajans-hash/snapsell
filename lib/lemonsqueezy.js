 ...
 *
 * Ortam (test / live) için ayrı bir API adresi yok: yalnızca https://api.lemonsqueezy.com/v1/…
 * Test veya canlı veri, hangi API anahtarını Bearer olarak gönderdiğinize bağlıdır (Lemon panelinde
 * test modu vs canlı mod anahtarları). Yanlış variant_id → 404; kod test URL’ine düşmez.
 */

const crypto = require("crypto");

/** Tek resmi API kökü — test/canlı ayrımı URL ile değil, API key ile yapılır. */
...
/**
 * sk_test_* / sk_live_* ve yaygın Lemon önekleri (lem_test_* / lem_live_*).
 * @param {string} apiKey
 * @returns {"test"|"live"|null}
 */
function inferLemonApiKeyEnvironment(apiKey) {
  const k = String(apiKey || "").trim().toLowerCase();
  if (!k) return null;
  if (k.startsWith("sk_test_")) return "test";
  if (k.startsWith("sk_live_")) return "live";
  if (k.startsWith("lem_test_")) return "test";
  if (k.startsWith("lem_live_")) return "live";
  return null;
}

/**
 * Tam anahtarı asla loglamayın. Önek + ortam özeti.
 * @param {string} apiKey
 * @returns {string}
 */
function formatLemonApiKeyForLog(apiKey) {
  ...
  if (!k) return "(empty)";
  const mode = inferLemonApiKeyEnvironment(apiKey);
  if (mode === "test") return "sk_test_… or lem_test_… (test — redacted)";
  if (mode === "live") return "sk_live_… or lem_live_… (live — redacted)";
  return "(prefix unrecognized — use test or live key from Lemon Developers → API)";
}

/**
 * GET /v1/variants/:id — checkout öncesi doğrulama (yanlış ID / test–live karışması).
 * @returns {Promise<{ ok: true, storeId: string, testMode: boolean|null } | { ok: false, status: number, detail: string }>}
 */
async function fetchVariantMeta(apiKey, variantId) {
  ...
  const testMode = attrs && typeof attrs.test_mode === "boolean" ? attrs.test_mode : null;
  return { ok: true, storeId: sid, testMode };
}
