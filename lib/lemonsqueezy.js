 ...
 * API tabanı: yalnızca https://api.lemonsqueezy.com/v1 (Lemon’un resmi uç noktası).
 * https://sandbox.lemonsqueezy.com veya benzeri bir adres bu projede kullanılmaz.
 */

const crypto = require("crypto");

/**
 * Tek HTTP tabanı — tüm fetch’ler buna bağlanır (variants, checkouts).
 * Canlı anahtar: Developers → API Keys. URL’de "sandbox" yoktur.
 */
...
/**
 * Yalnızca Lemon test modu API anahtarları (panelde test için üretilen).
 * Canlı anahtar: sk_live_*, lem_live_* veya çoğu zaman JWT ile başlayan eyJ… (Lemon’un varsayılan formatı).
 * @param {string} apiKey
 * @returns {boolean}
 */
function isLemonTestModeApiKey(apiKey) {
  const lower = String(apiKey || "").trim().toLowerCase();
  return lower.startsWith("sk_test_") || lower.startsWith("lem_test_");
}

/**
 * Tam anahtarı loglamayın.
 * @param {string} apiKey
 * @returns {string}
 */
function formatLemonApiKeyForLog(apiKey) {
  ...
  if (isLemonTestModeApiKey(k)) return "test API key prefix (redacted)";
  if (lower.startsWith("sk_live_") || lower.startsWith("lem_live_")) return "live API key sk_live_/lem_live_ (redacted)";
  if (k.startsWith("eyJ") || k.startsWith("eyj")) return "Lemon API key JWT eyJ… (live, redacted)";
  return "Lemon API key configured (redacted)";
}

/**
 * Variant JSON’dan mağaza ID: bazı yanıtlarda relationships.store.data yok; ürün üzerinden çözülür.
 * @param {object} variantData json.data
 * @returns {string|null}
 */
function storeIdFromVariantPayload(variantData) {
  if (!variantData) return null;
  const storeRel = variantData.relationships && variantData.relationships.store;
  const storeData = storeRel && storeRel.data;
  if (storeData && storeData.id != null) return String(storeData.id);
  return null;
}

/**
 * Ürün kaynağından store_id (Lemon dokümantasyonu: product.attributes.store_id).
 * @param {string} apiKey
 * @param {string|number} productId
 * @returns {Promise<string|null>}
 */
async function fetchStoreIdFromProduct(apiKey, productId) {
  const key = String(apiKey || "").trim();
  const pid = productId != null ? String(productId).trim() : "";
  if (!key || !pid) return null;
  const res = await fetch(LEMON_API_BASE + "/products/" + encodeURIComponent(pid), {
    method: "GET",
    headers: {
  ...
  const pdata = json && json.data;
  const attrs = pdata && pdata.attributes;
  if (attrs && attrs.store_id != null) return String(attrs.store_id);
  const srel = pdata && pdata.relationships && pdata.relationships.store && pdata.relationships.store.data;
  if (srel && srel.id != null) return String(srel.id);
  return null;
}

function productIdFromVariantPayload(variantData) {
  if (!variantData) return null;
  const prel = variantData.relationships && variantData.relationships.product && variantData.relationships.product.data;
  if (prel && prel.id != null) return String(prel.id);
  const attrs = variantData.attributes;
  if (attrs && attrs.product_id != null) return String(attrs.product_id);
  return null;
}

/**
 * GET /v1/variants/:id — checkout öncesi doğrulama. Test modundaki variant’lar reddedilir.
 * Store ID: variant.relationships.store veya ürün üzerinden (Lemon API çoğu zaman store’u variant’ta inline vermez).
...
async function fetchVariantMeta(apiKey, variantId) {
  ...
  const variantData = json && json.data;
  let sid = storeIdFromVariantPayload(variantData);
  if (!sid) {
    const productId = productIdFromVariantPayload(variantData);
    if (productId) {
      sid = await fetchStoreIdFromProduct(key, productId);
    }
  }
  if (!sid) {
    return {
      ...
      status: 500,
      detail:
        "Lemon variant için mağaza ID çözülemedi (variant.store veya product.store_id). Ürün/variant API yanıtını kontrol edin."
    };
  }
  const attrs = variantData && variantData.attributes;
...
async function createCheckoutOnce(opts) {
  ...
  if (!res.ok) {
    ...
    err.lemonBody = text.length > 65536 ? text.slice(0, 65536) + "…[truncated]" : text;
    throw err;
  }
