...
async function getPricesFromExa(productDescription) {
  ...
  try {
    ...
    for (const r of results) {
      ...
      if (content?.highlights?.length) textParts.push(content.highlights.join(" "));
      if (content?.text) textParts.push(content.text);
    }
    const combined = textParts.join(" ").trim();
    const hasContents = combined.length >= 50;
    if (!hasContents) {
      return {
        TRY: { minPrice: null, avgPrice: null, maxPrice: null, count: 0 },
        USD: { minPrice: null, avgPrice: null, maxPrice: null, count: 0 },
        dataLimited: true
      };
    }
    ...
    return { TRY: tryStats, USD: usdStats, dataLimited: false };
  } catch (e) {
...
async function getPricesFromSerpAPI(productQuery) {
  ...
  try {
    ...
    try {
      ...
      console.warn("SerpAPI fiyat: JSON parse hatası", parseErr?.message);
      return null;
    }
    if (!data || typeof data !== "object") return null;
    const shopping = data.shopping_results ?? [];
    const inline = data.inline_shopping_results ?? [];
    const categorized = Array.isArray(data.categorized_shopping_results) ? data.categorized_shopping_results : [];
    const hasAny = (Array.isArray(shopping) && shopping.length > 0) ||
      (Array.isArray(inline) && inline.length > 0) ||
      categorized.some((c) => Array.isArray(c?.shopping_results) && c.shopping_results.length > 0);
    ...
    (Array.isArray(shopping) ? shopping : []).forEach(collect);
    (Array.isArray(inline) ? inline : []).forEach(collect);
    categorized.forEach((cat) => (Array.isArray(cat?.shopping_results) ? cat.shopping_results : []).forEach(collect));
...
function mergePriceSources(scraperResult, exaPrices, serpapiPrices) {
  ...
  const dataLimited = !!(exaPrices && exaPrices.dataLimited === true);
  if (serpapiPrices) {
...
/** Builds robust output fields from platforms array: min_price, max_price, average_price, suggested_price, confidence_score. */
function robustOutputFromPlatforms(platforms) {
  if (!Array.isArray(platforms) || platforms.length === 0) {
    return { min_price: undefined, max_price: undefined, average_price: undefined, suggested_price: undefined, confidence_score: 0 };
  }
  const allPrices = [];
  for (const p of platforms) {
    if (p.minPrice != null) allPrices.push(p.minPrice);
    if (p.avgPrice != null) allPrices.push(p.avgPrice);
    if (p.maxPrice != null) allPrices.push(p.maxPrice);
  }
  const hasAny = allPrices.length > 0;
  const min_price = hasAny ? Math.min(...allPrices) : undefined;
  const max_price = hasAny ? Math.max(...allPrices) : undefined;
  const average_price = hasAny ? Math.round((allPrices.reduce((a, b) => a + b, 0) / allPrices.length) * 100) / 100 : undefined;
  const suggested_price = average_price != null ? Math.round(average_price * 0.95 * 100) / 100 : undefined;
  const confidence_score = hasAny ? Math.min(100, Math.max(0, 50 + allPrices.length * 5)) : 0;
  return { min_price, max_price, average_price, suggested_price, confidence_score };
}
...
export async function getPriceAnalysisUnified(imageUrl, userText, seoText) {
  ...
  if (process.env.SERPAPI_API_KEY && (imageUrl || fallbackText)) {
    try {
      ...
      const hasValidPrice = result?.platforms?.some(
        (p) => p.avgPrice != null || p.minPrice != null || p.maxPrice != null
      );
      if (result?.platforms && Array.isArray(result.platforms) && result.platforms.length > 0 && hasValidPrice) {
        const rob = robustOutputFromPlatforms(result.platforms);
        return {
          productName: result.productName || productNameFromQuery || "Ürün",
          platforms: result.platforms,
          summaryText: result.summaryText || "Fiyat analizi tamamlandı.",
          ...rob
        };
      }
    } catch (e) {
  ...
  const [scraperSettled, exaSettled, serpapiSettled] = await Promise.allSettled([
    getPriceAnalysisWithScraperAPI(effectiveQuery),
    getPricesFromExa(effectiveQuery),
    getPricesFromSerpAPI(effectiveQuery)
  ]);
  const scraperResult = scraperSettled.status === "fulfilled" ? scraperSettled.value : null;
  const exaPrices = exaSettled.status === "fulfilled" ? exaSettled.value : null;
  const serpapiPrices = serpapiSettled.status === "fulfilled" ? serpapiSettled.value : null;
  ...
  let summaryText = "";
  const dataLimited = merged && merged.dataLimited === true;
  if (process.env.OPENAI_API_KEY && platforms.length > 0) {
    try {
      const { default: OpenAI } = await import("openai");
      const openai = new OpenAI({ apiKey: process.env.OPENAI_API_KEY });
      const cleanedList = platforms
        .filter((p) => p.minPrice != null || p.avgPrice != null || p.maxPrice != null)
        .map((p) => {
          const fmt = (v) => (v == null ? "—" : p.currency === "TRY" ? v.toLocaleString("tr-TR", { minimumFractionDigits: 2 }) : v.toLocaleString("en-US", { minimumFractionDigits: 2 }));
          return `${p.name}: En düşük ${fmt(p.minPrice)}, Ortalama ${fmt(p.avgPrice)}, En yüksek ${fmt(p.maxPrice)} ${p.currency}`;
        });
      const tableDesc = cleanedList.join("\n");
      const systemContent = [
        "Eğer veriler çok çelişkiliyse (örneğin 100 TL ve 10.000 TL yan yanaysa), outlier temizliği yap ve sadece mantıklı kümelenmeye odaklan.",
        dataLimited ? "Veri kısıtlı, genel piyasa bilgisi kullan." : ""
      ].filter(Boolean).join(" ");
      const res = await openai.chat.completions.create({
        model: "gpt-4o-mini",
        messages: [
          ...
          {
            role: "user",
            content: `Ürün: "${productName}". Aşağıdaki fiyat listesi için 2-4 cümle Türkçe fiyat analizi raporu yaz: piyasa aralığı, bilgi amaçlı özet.\n\n${tableDesc || "Veri yok."}`
          }
        ]
      });
      summaryText = (res.choices?.[0]?.message?.content || "").trim();
  ...
  const rob = robustOutputFromPlatforms(platforms);

  let finalSummary = summaryText || "Fiyat verileri tabloya göre değerlendirilebilir.";
  if (rob.average_price != null || rob.suggested_price != null) {
    const parts = [];
    if (rob.average_price != null) parts.push(`Ortalama: ${rob.average_price}.`);
    if (rob.suggested_price != null) parts.push(`Önerilen satış fiyatı: ${rob.suggested_price}.`);
    finalSummary = parts.join(" ") + (summaryText ? " " + summaryText : "");
  }
  ...
  return {
    ...
    min_price: rob.min_price,
    max_price: rob.max_price,
    average_price: rob.average_price,
    suggested_price: rob.suggested_price,
    confidence_score: rob.confidence_score
  };
}
