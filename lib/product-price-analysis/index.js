...
import { toNumericPrices, applyStrongOutlierFilter } from "./price-cleaning.js";
...
export { cleanPrices, removeDuplicatePrices, removePriceOutliers, toNumericPrices, filterListingsByCleanedPrices, filterListingsByGlobalMedian, applyStrongOutlierFilter } from "./price-cleaning.js";
...
/**
 * Runs the full pipeline: relevance filter → strong outlier (0.5–1.8 × median) → confidence → output.
...
export async function runProductPriceAnalysis(imageUrl, options = {}) {
  ...
  const { accepted: afterRelevance, rejected: rejectedByRelevance } = filterByRelevance(rawListings, product, 0.2);

  const afterOutlier = applyStrongOutlierFilter(afterRelevance, 0.5, 1.8);
  ...
  const rejected_listings = [
    ...
    ...rejectedByOutlier.map((l) => ({
      ...
      reason: "Price outside valid range (0.5–1.8 × median)"
    }))
  ];

  const accepted_listings = afterOutlier;
  ...
  const platforms = buildPlatformTable(accepted_listings, 0.5);
...
export async function runForEditorTable(imageUrl, userHint = "", options = {}) {
  ...
  const result = imageUrl
    ? await runProductPriceAnalysis(imageUrl, { userHint, gl })
    : await runProductPriceAnalysisFromText(userHint, { gl });
