...
export async function searchPrices(query, options = {}) {
  ...
  try {
    ...
    try {
      ...
      console.warn("[product-price-analysis] price-search: JSON parse error", parseErr?.message);
      return [];
    }
    ...
    const hasAnyResults = (Array.isArray(shopping) && shopping.length > 0) ||
    ...
    (Array.isArray(shopping) ? shopping : []).forEach(add);
    (Array.isArray(inline) ? inline : []).forEach(add);
    for (const cat of categorized) {
      (Array.isArray(cat?.shopping_results) ? cat.shopping_results : []).forEach(add);
    }
...
/** Site-scoped query prefixes for marketplace-specific search (engine may still return mixed results). */
const PLATFORM_SITE_QUERIES = Object.freeze([
  { site: "site:amazon.com.tr", platform: "Amazon Turkey" },
  { site: "site:amazon.com", platform: "Amazon" },
  { site: "site:etsy.com", platform: "Etsy" },
  { site: "site:trendyol.com", platform: "Trendyol" },
  { site: "site:hepsiburada.com", platform: "Hepsiburada" },
  { site: "site:n11.com", platform: "N11" }
]);

/**
 * Runs separate searches per marketplace (site:domain + query), then merges listings.
 * Each result is tagged by platform from the site used. Use for per-platform analysis.
 * @param {string} query - Product search query (e.g. "beige shoulder bag women")
 * @param {Object} [options]
 * @param {string} [options.gl="tr"]
 * @param {number} [options.numPerPlatform=15]
 * @returns {Promise<import("./types.js").Listing[]>}
 */
export async function searchPricesPerPlatform(query, options = {}) {
  const { gl = "tr", numPerPlatform = 15 } = options;
  if (!SERPAPI_API_KEY || !query || typeof query !== "string") return [];
  ...
  const results = await Promise.all(
    PLATFORM_SITE_QUERIES.map(({ site }) => searchPrices(`${site} ${q}`, { gl, num: numPerPlatform }))
  );
  ...
  for (const list of results) {
    for (const item of list) {
      const key = `${item?.title ?? ""}|${item?.price ?? ""}|${item?.platform ?? ""}|${item?.url ?? ""}`;
