...
/** Minimum token-overlap similarity (0.75 = strict for maximum accuracy). */
...
export function scoreListing(product, listing) {
  ...
  if (tokenSim < MIN_TITLE_SIMILARITY) {
    return { score: 0, rejectReason: `Title similarity too low (${(tokenSim * 100).toFixed(0)}% < 75%)` };
  }
 ...
 * @param {import("./types.js").Product} product
 * @param {number} [minScore=0.75] - Minimum score to accept (token similarity product vs title >= 0.75)
 * @returns {{ accepted: import("./types.js").Listing[], rejected: import("./types.js").RejectedListing[] }}
 */
export function filterByRelevance(listings, product, minScore = 0.75) {
  const accepted = [];
  const rejected = [];
  const seen = new Set();
