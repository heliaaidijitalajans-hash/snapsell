...
const PRODUCT_TYPE_REJECT = Object.freeze([
  { productTerms: ["bag", "çanta", "handbag", "clutch"], rejectInTitle: ["wallet", "cüzdan", "luggage", "bavul", "suitcase"] },
  { productTerms: ["shoulder bag", "omuz çantası"], rejectInTitle: ["luggage", "bavul", "suitcase"] },
...
/**
 * Simple title similarity: share of category terms found in title (0..1).
 * @param {string} titleNorm
 * @param {string[]} categoryTerms
 * @returns {number}
 */
 ...
 * - product_type or category match is required (else reject).
 * - Brand match boosts; color/material match boosts.
...
export function scoreListing(product, listing) {
  ...
  if (!hasCategoryMatch(titleNorm, categoryTerms)) {
    return { score: 0, rejectReason: "Category/product_type does not match title" };
  }

  const sim = titleSimilarity(titleNorm, categoryTerms);
  if (sim < MIN_TITLE_SIMILARITY) {
    return { score: 0, rejectReason: "Title similarity too low" };
  }

  let score = sim;
  const brand = normalize(product?.brand ?? "");
  if (brand && titleNorm.includes(brand)) score += 0.3;
  const color = normalize(product?.color ?? "");
  if (color && titleNorm.includes(color)) score += 0.15;
  const material = normalize(product?.material ?? "");
  if (material && titleNorm.includes(material)) score += 0.1;
 ...
 * @param {import("./types.js").Product} product
 * @param {number} [minScore=0.2] - Minimum score to accept (after category match and reject checks)
 * @returns {{ accepted: import("./types.js").Listing[], rejected: import("./types.js").RejectedListing[] }}
 */
export function filterByRelevance(listings, product, minScore = 0.2) {
  const accepted = [];
  const rejected = [];
  const seen = new Set();
