...
async function sendSubscriptionWelcomeEmail(to, plan, credits) {
  ...
  const creditsStr = escapeHtml(String(credits));
  ...
  const from = String(process.env.RESEND_FROM || "SnapSell <onboarding@snapsell.website>").trim();
...
/** Senin örüğündeki isimle uyumlu alias */
